...
impl SstIndexApplier {
    ...
    pub async fn apply(&self, file_id: FileId) -> Result<ApplyOutput> {
        ...
        let puffin_reader = match self.cached_puffin_reader(file_id).await? {
            Some(puffin_reader) => puffin_reader,
            None => self.remote_puffin_reader(file_id).await?,
        };
    ...
    /// Helper function to create a [`SstPuffinReader`] from the cached index file.
    ...
    /// Helper function to create a [`SstPuffinReader`] from the remote index file.
    async fn remote_puffin_reader(&self, file_id: FileId) -> Result<SstPuffinReader> {
        ...
        puffin_manager.reader(&file_path).await.context(PuffinSnafu)
    }
}
