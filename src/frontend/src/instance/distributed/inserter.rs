...
use crate::error::{
    FindDatanodeSnafu, FindTableRouteSnafu, JoinTaskSnafu, RequestDatanodeSnafu,
    RequestInsertsSnafu, Result, SplitInsertSnafu,
};
...
impl DistInserter {
    ...
    pub(crate) async fn insert(&self, requests: InsertRequests) -> Result<u64> {
        ...
        let results = future::try_join_all(requests.into_iter().map(|(peer, inserts)| {
            ...
            // TODO(zhongzc): add trace_id to the request
            let _trace_id = self.ctx.trace_id();
                ...
                InsertRequest {
                    ...
                    rows: Some(Rows {
                        ...
                        rows: rows
                            ...
                            .map(|v| Row {
                                values: vec![Value {
                                    value_data: v.map(|i| ValueData::I32Value(i)),
                                }],
                            })
                            .collect(),
                    }),
