...
const NAME: &str = "cos_distance";

#[derive(Clone, Debug, Default)]
pub struct CosDistranceFunction;

impl Display for CosDistranceFunction {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", NAME.to_ascii_uppercase())
    }
}

impl Function for CosDistranceFunction {
    fn name(&self) -> &str {
        NAME
    }
    ...
    fn eval(&self, _func_ctx: FunctionContext, columns: &[VectorRef]) -> Result<VectorRef> {
        ...
        for i in 0..size {
            ...
            let d: f32 =
                <f32 as simsimd::SpatialSimilarity>::cos(&left_value, &right_value).unwrap() as f32;
