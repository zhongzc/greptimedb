...
fn test_create_region_request_template() {
    let procedure = CreateTableProcedure::new(
        ...
        test_data::new_ddl_context(DatanodeClientsRef::default()),
    );
...
async fn new_datanode_clients(
    region_server: &EchoRegionServer,
    region_routes: &[RegionRoute],
) -> DatanodeClientsRef {
    let clients = DatanodeClientsRef::default();
...
async fn test_on_datanode_create_regions() {
    ...
    let datanode_clients = new_datanode_clients(&region_server, &region_routes).await;

    let mut procedure = CreateTableProcedure::new(
        1,
        create_table_task(),
        region_routes,
        test_data::new_ddl_context(datanode_clients),
    );
...
fn test_create_alter_region_request() {
    ...
    let alter_region_request = procedure.create_alter_region_request(region_id);
...
async fn test_submit_alter_region_requests() {
    ...
    let context = test_data::new_ddl_context(datanode_clients);
