...
use mock_file_accessor::MockFileAccessor;
...
async fn test_put_get_file() {
    ...
    for compression_codec in compression_codecs {
        ...
        let mut file = reader.blob(key).await.unwrap();
        let mut buf = Vec::new();
        ...
        for (key, raw_data) in &blobs {
            writer
                .put_blob(
                    ...
                    *raw_data,
                    Some(PutOptions {
                        data_compression: compression_codec.clone(),
                    }),
                )
                .await
                .unwrap();
...
async fn test_put_get_dir() {
    ...
    for compression_codec in compression_codecs {
        ...
        for (file_key, raw_data) in &files_in_dir {
        ...
        let reader = puffin_manager.reader(puffin_file_name).await.unwrap();
        let res_dir = reader.dir(key).await.unwrap();
...
async fn test_put_get_mix_file_dir() {
    ...
    for compression_codec in compression_codecs {
        ...
        let puffin_manager = CachedPuffinManager::new(cache_manager, file_accessor.clone());
        ...
        let res_dir = reader.dir(dir_key).await.unwrap();
        for (file_name, raw_data) in &files_in_dir {
            let file_path = res_dir.join(file_name);
            let buf = std::fs::read(file_path).unwrap();
            assert_eq!(buf, *raw_data);
        ...
        let (_cache_dir, cache_manager) =
            new_moka_cache_manager("test_put_get_mix_file_dir_").await;
        let puffin_manager = CachedPuffinManager::new(cache_manager, file_accessor);
