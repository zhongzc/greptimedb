...
impl Encoder for IntermediateItemEncoderV1 {
    type Item<'a> = (Bytes, RoaringBitmap);
    type Error = Error;

    fn encode(&mut self, item: (Bytes, RoaringBitmap), dst: &mut BytesMut) -> Result<()> {
        let value_bytes = item.0;
        let mut bitmap_bytes = vec![];
        item.1.serialize_into(&mut bitmap_bytes)?;
...
impl Decoder for IntermediateItemDecoderV1 {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>> {
        ...
        let bitmap = RoaringBitmap::deserialize_from(&buf[..bitmap_len]).unwrap();
